...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # retrieve the new post we added and store it back in the var
    db.refresh(new_post)
...
def update_post(id: int, post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # don't allow user to edit posts that aren't theirs
    if updated_post.first().owner_id != current_user.id:
        raise HTTPException(status.HTTP_403_FORBIDDEN, detail='Not authorized to perform that action.')
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # don't allow user to delete posts that aren't theirs
